...
BOOKING_CREATE_LOCK = threading.Lock()
BOOKING_CANCEL_LOCK = threading.Lock()
...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    ...
    for b in existing:
        ...
        # Use strict < to allow back-to-back bookings.
...
def cancel_booking(
    ...
    with BOOKING_CANCEL_LOCK:
        ...
        notice_seconds = notice.total_seconds()
        if notice_seconds >= 48 * 3600:
            refund_percent = 100
        elif notice_seconds >= 24 * 3600:
            refund_percent = 50
        else:
            refund_percent = 0
